...
a x d(m,t,n)|m==0=[0.7,0.6,1]%((1-(
 ...
 in if mod(z!!0+z!!1)2>0then[3,1,1]%s
  else[3,3,3]%s where
 h=x&(d%t);l=o$[9,9,16]&(h%(-1))
 r=d&(n%((n#d)*(-2)));g=l#n;f(e,_,_)=e
 b=if g<0||(f$i h l)/=0then 0else g
 p=if b>0then(l#r)**99else 0
p(_,e,_)=e;h r d z=if l>0&&s>0.01then(2
 ,s,o$p&(d%s))else(2,1e7,[])where
 p=r&z;b=p#d;l=b*b-(p#p)+1;s= -b-sqrt l
n a b=if p a<=p b then a else b
m x=foldl1 n x;i r d=m$c:if p>0.01then
 [(1,p,[0,0,1])]else[(0,1e6,[])]
 where p= -(r!!2)/(d!!2);c=m$k(h r d)j
e a b c t=let z=[-28,0,28]in(foldl1
 (&)[(f a b c t((a%v)&(b%w)))|v<-z,w<-
 z])%7;f a b c(x,y)t=
 let d=o$(t%(-1))&(((a%x)&(b%y)&c)%16)
     r=[17,16,8]&t in(q r d)%3.5;main=
